...
SessionPtr Session::create(ContactPtr contact_p, uint32_t sid) 
{
  return SessionPtr(new Session(contact_p, sid));
}

void Session::send(uint32_t channel, uint32_t command, std::vector<char>& message) const
{
